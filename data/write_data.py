...
import pickle
import csv
from wf_demo.default_load import input_dict, load_default_latent_tensor, save_realization_latent_tensor
...
print(sim.all_data_types)
if sim.all_data_types is ['point']:
    datatype = 'point'
else:
    datatype = 'UDAR'
...
if datatype == 'UDAR':
    for component in range(60, 60+1):
        upper = 60
        if component >= upper:
            upper = component+2
        for end_comp in range(component+1, upper, 7):
            ...
            if component < my_size:
                ...
                save_realization_latent_tensor(my_latent_tensor, f"C1_minus_{component}_{end_comp}")


            index_vector = torch.full((1, 40), fill_value=32, dtype=torch.long).to(device)
            ...
            gs = GridSpec(4, 2, width_ratios=[20, 1], height_ratios=[0.5, 1, 2, 1.2], figure=fig)

            # Left column: main plots
            ax = fig.add_subplot(gs[0, 0])
            ax_img = fig.add_subplot(gs[1, 0])
            ax_res = fig.add_subplot(gs[2, 0], sharex=ax_img)
            ax_logs = fig.add_subplot(gs[3, 0], sharex=ax_img)

            # Right column: color-bars
            ax_img_cbar = fig.add_subplot(gs[1, 1])
            ax_res_cbar = fig.add_subplot(gs[2, 1])
            ax_logs_cbar = fig.add_subplot(gs[3, 1])

            # axes
            ax.axhline(0)
            ax.axvline(0)

            v = my_latent_tensor[0].cpu().numpy()

            v2 = v[2:]

            # main 2D arrow (first two components)
            ax.arrow(0, 0, v2[0], v2[1],
                    head_width=abs(v2[1] / 10.), length_includes_head=True)

            # label at head
            ax.text(v2[0], v2[1],
                    f"(..., {v2[0]:.2f}, {v2[1]:.2f}, {v2[2]:.2f}, {v2[3]:.2f}, {v2[4]:.2f}, ...)",
                    ha="left", va="bottom")
            ...
            im_res_1 = ax_res.imshow(img_res, extent=(-0.5, num_cols_res - 0.5, resistivity.shape[2], 0),
                        aspect='auto', interpolation='none', cmap='summer')
            ax_res.set_title("Resistivity padded input (h-component)")
            ...
            # add colorbar
            cbar = fig.colorbar(im_res_1, cax=ax_res_cbar)
            cbar.set_label("Resistivity (Ohm m)")
            # TODO there is some bug with the color scale when there is no sand
            ...
            # saving
            if component < my_size:
                fig.savefig(f'SynthTruth_updated_{component}_{end_comp}.png', bbox_inches='tight', dpi=300)
            else:
